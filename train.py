...
def convert_xml_annotations(annotation_paths: list[Path], output_dir: Path, image_dir: Path | None = None) -> list[Path]:
    ...
    for annotation_path in annotation_paths:
        ...
        for obj in root.findall("object"):
            ...
            class_id = 0 if name == "helmet" else 1
            if name not in {"helmet", "face mask"}:
                continue
...
def main() -> None:
    ...
    if args.prepare_only:
        logging.info("Dataset prepared successfully at %s", data_path)
        return
